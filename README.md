...
Description:
This function takes in one parameter, which will be another function. When given an input string, this function will pass the input string to its child function and return an output object with a matchFound property which will have the opposite truth value to the one produced by its child node.
