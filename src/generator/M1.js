class M1{
  ...
  static Export(matchNode){
    ...
    let nodesAsList = M1.GetNodesAsList(matchNode)
    ...
    for (let sortedNode of sortedNodes){
      outputString += H1.EncodeDepth(1) + sortedNode.serial + '\n'
      for (let key of Object.keys(sortedNode)){
        outputString += H1.EncodeDepth(2) + key + '\n' 
        let tempString = ''

        switch (key){
          case 'subMatches':
            for (let subMatch of sortedNode['subMatches']){
              tempString += H1.EncodeDepth(3) + subMatch.serial + '\n'
            }
            break
          case 'parent':
            {
              ...
              tempString = H1.EncodeDepth(3) + parentNodeId + '\n'
              break
            }
          default:
            tempString = H1.EncodeDepth(3) + sortedNode[key] + '\n'
            break
        }
